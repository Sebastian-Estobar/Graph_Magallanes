...
// ==========================
// FUNCIONES DE CONTROL DE NODO COMPACTO Y ULTRA-COMPATIBLE
// ==========================
function nodeActive(nodeId) {
    ...
    if ($("#attributepane").length > 0) {
        // Aseguramos que los contenedores nativos estén visibles por reglas CSS
        $(".nodeattributes .p, .nodeattributes .link").show();
        
        // Colocamos el nombre principal de la organización
        $("#attributepane .name").html(node.label);
        
        // 1. Inyección compacta de Atributos (evita que ocupe demasiado alto)
        ...
        // 2. Inyección forzada en el contenedor original de la plantilla para respetar el scroll del CSS
        var connectionsHtml = "";
        sigInst.iterEdges(function(edge) {
            if (edge.source === nodeId || edge.target === nodeId) {
        ...
        // Forzamos la actualización del título y la lista donde style.css espera encontrarlos
...
// ==========================
// CONTROL DE LA BOTONERA DE ZOOM
// ==========================
